...
In the past year, the real OkCupid introduced [new choices for gender identities and sexual preferences](http://www.huffingtonpost.com/2014/11/17/okcupid-new-gender-options_n_6172434.html) that extend far beyond the traditional binary male/female.  From a data architect's perspective this could be a nightmare.  Adding a new field for each possible gender is unwieldy.  In Cosmo-Connect the structure is normalized so this isn't a problem.  A user chooses one gender from a list, and can only pick one.  Then to indicate orientation, users check the boxes "interested_in" for one or multiple genders.  This makes the database scalable as dating websites increasingly cater to clients' more nuanced gender identities and sexual orientations.
